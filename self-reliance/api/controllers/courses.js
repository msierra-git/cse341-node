...
const findAllCourses = async (req, res) => {
   /*
    #swagger.description = 'This will get all self-reliance course records or documents <br/><br>'	 	
   */
   try {
...
const findOneCourse = async (req, res) => {
   /*
    #swagger.description = 'This will find one self-reliance course based on the custom ID called courseID.'	 	
   */
   try {
...
const createCourse = async (req, res) => {
   /*
    #swagger.description = 'This will create one self-reliance course using the data in JSON format.<br>It will then return the ID of the new record if operation is successful.'	 	
   */
   try {
      const newCourse = {
      ...
      if (result.acknowledged) {
         ...
         console.log(`New SR Course created with new ID: ${result.insertedId}`);
      } else {
...
const updateCourse = async (req, res) => {
   /*
    #swagger.description = 'This will update the details of the self-reliance course based on the supplied custom ID called courseID.<br>It will use the new information as supplied in JSON format.'	     
   */
   try {
      // const courseID = new ObjectID(req.params.courseID);
      const courseID = parseInt(req.params.courseID, 10);
      ...
      const result = await mongoDB
         ...
         .db('self_reliance')
         .collection('courses')
         .replaceOne({ courseID: courseID }, newCourse);

      if (result.modifiedCount > 0) {
         res.status(204).send();
         console.log(`Contact SR Course with user ID: ${courseID}`);
      } else {
...
const deleteCourse = async (req, res) => {
   /*
    #swagger.description = 'This will remove the self-reliance course based on the supplied custom ID called courseID.'	 	
   */
   try {
      // const courseID = new ObjectID(req.params.courseID);
      const courseID = parseInt(req.params.courseID, 10);
      ...
      if (result.deletedCount > 0) {
         res.status(200).send();
         console.log(`SR Course deleted with ID: ${courseID}`);
      } else {
